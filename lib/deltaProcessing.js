  ...
  for (const { subject, type } of subjectsWithTypes) {
    ...
    if (
      ...
      for (const organisationUUID of organisationUUIDs) {
        ...
        await moveSubjectBetweenGraphs(
          subject,
          insertGraph,
          organisationGraphs,
        );
...
async function getOrganisationUUIDs(subject, type) {
  ...
  for (const pathConfig of pta) {
    if (pathConfig.type.value === type.value) {
      let queryMunicipalityName =
        '?association <http://www.w3.org/ns/org#hasPrimarySite>/<https://data.vlaanderen.be/ns/organisatie#bestaatUit>/<https://data.vlaanderen.be/ns/adres#gemeentenaam> ?gemeentenaam .';
      if (
        'https://data.vlaanderen.be/ns/FeitelijkeVerenigingen#FeitelijkeVereniging' ===
        type.value
      ) {
        queryMunicipalityName =
          '?subject <http://www.w3.org/ns/org#hasPrimarySite>/<https://data.vlaanderen.be/ns/organisatie#bestaatUit>/<https://data.vlaanderen.be/ns/adres#gemeentenaam> ?gemeentenaam .';
      }
      const response = await mas.querySudo(`
        ${env.SPARQL_PREFIXES}
